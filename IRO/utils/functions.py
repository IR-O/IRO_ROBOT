...
from IRO import aiohttpsession as aiosession
from IRO.services.dbfunctions import start_restart_stage
from IRO.utils.http import get, post


async def restart(m: Message):
    if m:
        await start_restart_stage(m.chat.id, m.message_id)
    execvp(executable, [executable, "-m", "IRO"])
...
def generate_captcha():
    ...
    height = 100
    correct_answer = ""
    font = ImageFont.truetype("IRO/IROFont/logofont", 55)
    file = f"IRO/IROFont/{randint(1000, 9999)}.jpg"
